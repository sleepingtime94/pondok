...
html, body {
  height: 100%;
  overflow: hidden;          /* scrollbar hanya di chat-body */
...
.chat-container {
  width: 100%;
  max-width: 900px;
  height: 100%;              /* isi tinggi wrapper */
  margin: 0 auto;
  display: flex;
  flex-direction: column;
  overflow: hidden;          /* tidak boleh scroll di sini */
}
...
.chat-header {
  ...
  font-size: 1.1rem;
  font-weight: 600;
  flex: 0 0 auto;            /* tetap di atas */
  box-shadow: 0 2px 4px rgba(0,0,0,.1);
}

/* ========== BODY / PESAN ========== */
.chat-body {
  flex: 1 1 auto;            /* sisa ruang tinggi */
...
.chat-body::-webkit-scrollbar-thumb { background: #aaa; border-radius: 4px }

/* ========== BUBBLE ========== */
.message-wrapper { display: flex; margin-bottom: 15px; width: 100%; align-items: flex-start; }
.message-wrapper.me { justify-content: flex-end; }
.message-wrapper.admin { justify-content: flex-start; }

.message-container { max-width: 50%; display: flex; flex-direction: column; }
...
.message-wrapper.me .message-date { text-align: right; }
.message-wrapper.admin .message-date { text-align: left; }
...
.date-divider::before {
  ...
  height: 1px; background: #ccc; z-index: 0;
}
...
.chat-footer input:focus { border-color: #128c7e; }
.chat-footer button {
  border: none;
  background: #128c7e;
  color: #fff;
...
.chat-footer button:hover { background: #075e54; }

/* ========== RESPONSIVE ========== */
@media(max-width:768px){
  .chat-container { max-width: 100%; border-radius: 0; }
  .message-container { max-width: 70%; }
  .chat-body { padding: 10px; }
  .chat-footer { padding: 0 10px; }
}
@media(max-width:576px){
  :root{ --h-header:50px; --h-footer:56px; }
  .chat-header { font-size: 1rem; }
  .chat-footer input { padding: 10px 14px; font-size: 14px; }
  .chat-footer button { width: 40px; height: 40px; }
  .chat-footer button svg { width: 20px; height: 20px; }
...
<script>
  ...
  socket.on('connect', () => {
    socket.emit('register', {id: `${USER_ID}`, username: '{{ auth()->user()->name }}', role: 'user'});
});
